...
def _wrapped_execution(ext, data, generated_at, min_group=30, *args, **kwargs):
    ...
    PORT_OUT.write_text(json.dumps(portfolio_summary, ensure_ascii=False, indent=2), encoding="utf-8")

    ranking_summary = build_ranking_summary(trades, ext, data, generated_at)
    RANK_OUT.write_text(json.dumps(ranking_summary, ensure_ascii=False, indent=2), encoding="utf-8")

    ma5_summary = build_ma5_summary(data, generated_at)
    MA5_OUT.write_text(json.dumps(ma5_summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main():
    ...
    if hb.OUT.exists():
        ...
        if MA5_OUT.exists():
            ma5 = json.loads(MA5_OUT.read_text(encoding="utf-8"))
            summary["ma5TimingResearch"] = {
                "version": ma5.get("version"),
                "outputFile": str(MA5_OUT),
                "ruleDefinition": ma5.get("ruleDefinition"),
                "promotionPolicy": ma5.get("promotionPolicy"),
            }
